...
def AP_mapping(bankData_AP, bankData, glData_vendor, glData, map_vendor_byEntity, account_cd, list_bankCharge, nameNum, vendorStaff):
    ...
    for vendor in second_vendor_list:
        ...
        if len(glValue_list_vendor) >= 15:
            df_gl_vendor = glData_vendor_left1.loc[glData_vendor_left1['Vendor Name'] == f'{vendor}']
            set_JE = set(df_gl_vendor['JE Header Id'].to_list())
            subsets_JE = get_sub_set(list(set_JE))
            subsets_glIndex_vendor = list()
            for subset_JE in subsets_JE:
                glIndex_JEsubset = list(df_gl_vendor.loc[df_gl_vendor['JE Header Id'].isin(subset_JE)].index.values)
                subsets_glIndex_vendor.append(glIndex_JEsubset)
        else:
    ...
    for vendor, df in glData_vendor_left2.groupby('Vendor Name'):
        ...
        if len(df) >=15:
            set_JE = set(df['JE Header Id'].to_list())
            subsets_JE = get_sub_set(list(set_JE))
            subsets_glIndex_vendor_2 = list()
            for subset_JE in subsets_JE:
                glIndex_JEsubset = list(df.loc[df['JE Header Id'].isin(subset_JE)].index.values)
                subsets_glIndex_vendor_2.append(glIndex_JEsubset)
        else:
...
def reimbursement_mapping(bankData_potentialTS, bankData_TSBatch, bankData, glData_reimbursement, glData, df_reimPay, account_number, month_period):
    ...
    for month in set(df_reimPay_filtered['Month'].to_list()):
        ...
        for staff in list_staffName:
            ...
            if sum_pay + sum_gl_reim < 0.02 and sum_pay + sum_gl_reim > -0.02:
                ...
                if len(glData_reim_staffperM)>=15:
                    ...
                    for subset_JE in subsets_JE:
                        glIndex_JEsubset = list(glData_reim_staffperM.loc[glData_reim_staffperM['JE Header Id'].isin(subset_JE)].index.values)
            ...
            for number_PIR in list_PIRnumber:
                ...
                if len(bkValue_list_reim) >= 2:
                    ...
                    if len(index_in_bkTSBatch) == 1:
                        id_bkRef += 1
                        bk_perPIR_mapped = True
                        bkMappedIndex_to_PIR = dict.fromkeys(index_in_bkTSBatch, f'{number_PIR}')
                        exactMappedIndex_to_PIR.update(bkMappedIndex_to_PIR)
        ...
        if (False in bk_mapped) and (False not in gl_mapped):
            ...
            for key in valueMappedIndex_to_PIR.keys():
                ...
                bankData.loc[key, 'Category'] = f'reimbursement - value'
                bankData.loc[key, 'Identification'] = f'(reimbursement payment) ({now}) ({month}) (PIR{valueMappedIndex_to_PIR[key]})'
            ...
            mapped_bankIndex_reim = mapped_bankIndex_reim + bk_mapped_index
            mapped_glIndex_reim = mapped_glIndex_reim + gl_mapped_index
        if (False not in bk_mapped) and (False in gl_mapped):
            for key in exactMappedIndex_to_PIR.keys():
                bankData.loc[bk_mapped_index, 'Result'] = f'check payment in gl'
                bankData.loc[bk_mapped_index, 'Category'] = f'reimbursement'
                bankData.loc[bk_mapped_index, 'Identification'] = f'(reimbursement payment) ({now}) ({month}) (PIR{exactMappedIndex_to_PIR[key]})'
            for key in valueMappedIndex_to_PIR.keys():
                bankData.loc[key, 'Result'] = f'check payment in gl'
                bankData.loc[key, 'Category'] = f'reimbursement - value'
                bankData.loc[key, 'Identification'] = f'(reimbursement payment) ({now}) ({month}) (PIR{valueMappedIndex_to_PIR[key]})'
            glData.loc[gl_mapped_index, 'Result'] = f'check payment in gl'
            glData.loc[gl_mapped_index, 'Category'] = f'reimbursement'
            glData.loc[gl_mapped_index, 'Identification'] = f'(reimbursement payment) ({now}) ({month})'
            mapped_bankIndex_reim = mapped_bankIndex_reim + bk_mapped_index
            mapped_glIndex_reim = mapped_glIndex_reim + gl_mapped_index
        if (False in bk_mapped) and (False in gl_mapped):
            for key in exactMappedIndex_to_PIR.keys():
                bankData.loc[bk_mapped_index, 'Result'] = f'check TS Ref'
                bankData.loc[bk_mapped_index, 'Category'] = f'reimbursement'
                bankData.loc[bk_mapped_index, 'Identification'] = f'(reimbursement payment) ({now}) ({month}) (PIR{exactMappedIndex_to_PIR[key]})'
            for key in valueMappedIndex_to_PIR.keys():
                bankData.loc[key, 'Result'] = f'check TS Ref - value'
                bankData.loc[key, 'Category'] = f'reimbursement - value'
                bankData.loc[key, 'Identification'] = f'(reimbursement payment) ({now}) ({month}) (PIR{valueMappedIndex_to_PIR[key]})'
            glData.loc[gl_mapped_index, 'Result'] = f'check TS Ref'
            glData.loc[gl_mapped_index, 'Category'] = f'reimbursement'
            glData.loc[gl_mapped_index, 'Identification'] = f'(reimbursement payment) ({now}) ({month})'
            mapped_bankIndex_reim = mapped_bankIndex_reim + bk_mapped_index
            mapped_glIndex_reim = mapped_glIndex_reim + gl_mapped_index
    ...
    return mapped_bankIndex_reim, mapped_glIndex_reim
...
def df_to_endfile(df, file_path):
    ...
    worksheet.set_column('A:AP', 12, content_format)
...
try:
    ...
    path_folder_BS = r'C:\Users\he kelly\Desktop\bank_reconciliation_py\Test\2023.7\BS'
    path_folder_GL = r'C:\Users\he kelly\Desktop\bank_reconciliation_py\Test\2023.7\GL'
    path_folder_reimRegister = r'C:\Users\he kelly\Desktop\bank_reconciliation_py\Test\2023.7\REIM'
    directory_AP_Vendor = r'C:\Users\he kelly\Desktop\bank_reconciliation_py\Test\Mapping\AP Mapping.xlsx'
    directory_AP_Employee = r'C:\Users\he kelly\Desktop\bank_reconciliation_py\Test\Mapping\Employee mapping.xlsx'
    directory_Commercial = r'C:\Users\he kelly\Desktop\bank_reconciliation_py\Test\Mapping\Cash receipt 2023 (3).xlsx'
    month_period = 'JUL'
    path_folder_target = r'C:\Users\he kelly\Desktop\bank_reconciliation_py\Test\2023.7\RESULT'
    ...
    for column in df_bank.columns:
        if 'Unnamed' in column:
            df_bank = df_bank.drop(columns=f'{column}')
    ...
    for column in df_GL.columns:
        if 'Unnamed' in column:
            df_GL = df_GL.drop(columns=f'{column}')
    ...
    # bank_mapping_PRC = {'626-055784-001': '101001', '001-221076-031': '101245'}
    ...
    for account_number, account_cd in bank_mapping_PRC.items():
        ...
        staff_invoice_indication = ['HLYERR', 'TB', 'RVCR', 'CM', 'CR', 'NTS', 'UPLINVPayroll']
        ...
        mapped_bankIndex_reim, mapped_glIndex_reim = reimbursement_mapping(bankData_potentialTS, bankData_TSBatch, bankData, glData_reimbursement, glData, df_reimPay, account_number, month_period)
        ...
        for vendor, df_vendor in glData_beforeVM_vendor.groupby('Vendor Name'):
            ...
            if len(dic_vendor) >= 15:
                subsets_glIndex_vendor = [list(dic_vendor.keys())]
            else:
            ...
            subsets_glIndex_vendor.reverse()
            for subset_glIndex_vendor in subsets_glIndex_vendor:
                subset_glValue_vendor = key_to_value(subset_glIndex_vendor, dic_vendor)
                sum_glValue = sum(subset_glValue_vendor)
                for ind, row in bankData_beforeVM.iterrows():
                    bkValue = row['Credit/Debit amount']
                    mapped1 = False
                    if account_cd == '101245' and (abs(round((bkValue - sum_glValue), 2)) in list_bankCharge):
                        mapped1 = True
                    if account_cd != '101245' and abs(round((bkValue - sum_glValue), 2)) < 0.03:
                        mapped1 = True
                    if mapped1:
                        if (ind in mapped_bkIndex_value) or common_data(subset_glIndex_vendor, mapped_glIndex_value):
                            continue
                        id_number_value += 1
                        mapped_bkIndex_value.append(ind)
                        ...
                        bankData.loc[ind, 'Result'] = f'netoff - value'
                        bankData.loc[ind, 'Category'] = f'value'
                        bankData.loc[ind, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                        break

        print('mapped_bkIndex_value', mapped_bkIndex_value)
        print('mapped_glIndex_value', mapped_glIndex_value)
        excel_log.log(bankData_beforeVM, 'bankData_beforeVM')

        bankData_beforeVM = bankData_beforeVM.loc[bankData_beforeVM.index.difference(mapped_bkIndex_value)]

        for ind_gl, row_gl in glData_beforeVM_noVendor.iterrows():
            glValue = row_gl['Amount Func Cur']
            for ind_bk, row_bk in bankData_beforeVM.iterrows():
                bkValue = row_bk['Credit/Debit amount']
                mapped2 = False
                if account_cd == '101245' and (abs(round((bkValue - glValue), 2)) in list_bankCharge):
                    mapped2 = True
                if account_cd != '101245' and abs(round((bkValue - glValue), 2)) < 0.03:
                    mapped2 = True
                if mapped2:
                    if (ind_bk in mapped_bkIndex_value) or (ind_gl in mapped_glIndex_value):
                        continue
                    id_number_value += 1
                    mapped_bkIndex_value.append(ind_bk)
                    mapped_glIndex_value.append(ind_gl)
                    glData.loc[ind_gl, 'Result'] = f'netoff - value'
                    glData.loc[ind_gl, 'Category'] = f'value'
                    glData.loc[ind_gl, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                    bankData.loc[ind_bk, 'Result'] = f'netoff - value'
                    bankData.loc[ind_bk, 'Category'] = f'value'
                    bankData.loc[ind_bk, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                    break
        ...
        #     if 'Unnamed' in column:
        #         bankData = bankData.drop(columns=f'{column}')
    ...
    for account_number, account_cd in bank_mapping_HK.items():
        ...
        glData_beforeVM_vendor = glData_beforeVM[
            glData_beforeVM['Journal Source'].str.contains('Payables', case=False) | glData_beforeVM[
                'Journal Source'].str.contains(
                'Time & Bill', case=False)]
        glData_beforeVM_noVendor = glData_beforeVM[
            glData_beforeVM['Journal Source'].str.contains('Spreadsheet', case=False)]

        id_number_value = 0
        mapped_glIndex_value = []
        mapped_bkIndex_value = []

        for vendor, df_vendor in glData_beforeVM_vendor.groupby('Vendor/Client Name'):
            print(vendor)
            dic_vendor = df_vendor['Amount'].to_dict()
            subsets_glIndex_vendor = get_sub_set(dic_vendor.keys())
            print(subsets_glIndex_vendor)
            subsets_glIndex_vendor = [x for x in subsets_glIndex_vendor if x != []]
            for subset_glIndex_vendor in subsets_glIndex_vendor:
                ...
                for ind, row in bankData_beforeVM.iterrows():
                    bkValue = row['Credit/Debit amount']
                    if abs(bkValue - sum_glValue) < 0.03:
                        if (ind in mapped_bkIndex_value) or common_data(subset_glIndex_vendor, mapped_glIndex_value):
                            continue
                        id_number_value += 1
                        mapped_bkIndex_value.append(ind)
                        ...
                        bankData.loc[ind, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                        break
                    if ('/EXCH/' in row['Narrative']) and abs(round(bkValue - sum_glValue)) < abs(bkValue * 0.05):
                        if (ind in mapped_bkIndex_value) or common_data(subset_glIndex_vendor, mapped_glIndex_value):
                            continue
                        id_number_value += 1
                        mapped_bkIndex_value.append(ind)
                        mapped_glIndex_value = mapped_glIndex_value + subset_glIndex_vendor
                        bankData.loc[ind, 'Result'] = f'netoff - value/FX'
                        bankData.loc[ind, 'Category'] = f'value'
                        bankData.loc[ind, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                        bankData.loc[ind, 'FX'] = bkValue - sum_glValue
                        glData.loc[subset_glIndex_vendor, 'Result'] = f'netoff - value/FX'
                        glData.loc[subset_glIndex_vendor, 'Category'] = f'value'
                        glData.loc[subset_glIndex_vendor, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                        glData.loc[subset_glIndex_vendor, 'FX'] = (bkValue - sum_glValue) / len(subset_glIndex_vendor)
                        break
        ...
        for ind_gl, row_gl in glData_beforeVM_noVendor.iterrows():
            glValue = row_gl['Amount']
            for ind_bk, row_bk in bankData_beforeVM.iterrows():
                bkValue = row_bk['Credit/Debit amount']
                if abs(bkValue - glValue) < 0.03:
                    if (ind_bk in mapped_bkIndex_value) or (ind_gl in mapped_glIndex_value):
                        continue
                    id_number_value += 1
                    mapped_bkIndex_value.append(ind_bk)
                    ...
                    bankData.loc[ind_bk, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                    break
                if ('/EXCH/' in row_bk['Narrative']) and abs(round(bkValue - glValue)) < abs(bkValue * 0.05):
                    if (ind_bk in mapped_bkIndex_value) or (ind_gl in mapped_glIndex_value):
                        continue
                    id_number_value += 1
                    mapped_bkIndex_value.append(ind_bk)
                    mapped_glIndex_value.append(ind_gl)
                    bankData.loc[ind_bk, 'Result'] = f'netoff - value/FX'
                    bankData.loc[ind_bk, 'Category'] = f'value'
                    bankData.loc[ind_bk, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                    bankData.loc[ind_bk, 'FX'] = bkValue - glValue
                    glData.loc[ind_gl, 'Result'] = f'netoff - value/FX'
                    glData.loc[ind_gl, 'Category'] = f'value'
                    glData.loc[ind_gl, 'Identification'] = f'(value netoff) ({now}) ({id_number_value})'
                    glData.loc[ind_gl, 'FX'] = (bkValue - glValue) / len(subset_glIndex_vendor)
                    break

        glData_valueMapped = glData[glData['Category'] == 'value']
        if 'FX' in glData.columns:
            column_list_GL = ['Vendor/Client Name', 'Reference', 'Description', 'Amount', 'FX', 'Identification']
        else:
            column_list_GL = ['Vendor/Client Name', 'Reference', 'Description', 'Amount', 'Identification']
        bankData = bankData.merge(glData_valueMapped[column_list_GL], how='left', on='Identification')
        ...
        #         bankData = bankData.drop(columns=f'{column}')
        bankData['Value date'] = bankData['Value date'].apply(lambda x: x.strftime('%d/%m/%Y'))
        glData['Posted Date'] = glData['Posted Date'].apply(lambda x: x.strftime('%d/%m/%Y'))
